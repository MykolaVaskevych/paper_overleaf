...
%[P5: Search Spaces]
The search space for the game of Connect 4 is $10^{12}$ and for chess is $10^{40}$. It is difficult to enumerate each board in such large search spaces using brute force approach such as breadth first or depth first search algorithms. {\color{red} But this was achieved by IBM’s Deep Blue supercomputer, which beat chess world champion Garry Kasparov in 1996 --- a system that required purpose-built hardware costing millions of dollars and was inaccessible to all but a handful of organisations.} However, the search space for other games such as Go is significantly larger and brute search approaches do not work. Instead one has to use function approximators and deep learning architectures in particular given the recent success of Convolutional Neural Networks (CNNs) in computer vision. 
...
\textbf{RQ:} How does the performance of value-based Deep Q-Network agents compare to actor-critic agents (specifically Proximal Policy Optimization (PPO) and Advantage Actor-Critic (A2C)) in discrete action space environments, including their sample efficiency and training consistency during training?
...
{\color{red} While studies comparing these algorithm families exist \citep{defuente2024comparative}, they typically involve per-algorithm hyperparameter tuning, making it difficult to separate algorithmic capability from configuration effort. This project takes a different approach: Stable-Baselines3 default hyperparameters are used for every algorithm across all three experiments, with no tuning of any kind. The importance of this distinction is illustrated by \citet{mnih2015human}, who acknowledge that their published DQN results relied on hyperparameters ``selected by performing an informal search,'' noting that ``it is conceivable that even better results could be obtained by systematically tuning the hyperparameter values.'' All results in this thesis reflect what each algorithm delivers out of the box --- an intentional choice to measure inherent algorithmic behaviour rather than tuning effort.}
