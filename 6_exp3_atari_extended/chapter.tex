...
30 runs total (2 algorithms $\times$ 3 environments $\times$ 10 seeds). Training timing for the original run was lost due to an instrumentation issue; all reported wall-clock figures come from retrained reproducibility runs. DQN timing is fully recovered (30/30 seeds). PPO timing is 23/30 seeds: Seaquest is complete (10/10), Pong has 7/10 seeds, Breakout has 6/10 seeds (7 seeds permanently lost across both environments). This affects only the timing table; evaluation scores are complete for all 30 runs.

Wall-clock totals on RTX~4090: DQN $\approx$72.5\,h total (27.7\,h Pong, 17.4\,h Breakout, 27.4\,h Seaquest); PPO $\approx$105.9\,h estimated (25.8\,h Pong measured, ${\approx}$32.3\,h Breakout extrapolated, 36.8\,h Seaquest). See Table~\ref{tab:exp3-timing} for per-seed detail.
...
\begin{figure}[htbp!]
  ...
  \caption[Per-seed wall-clock training time: Pong, Breakout, Seaquest]{Per-seed wall-clock training time (hours) for all recovered seeds.
           DQN timing is complete (30/30 seeds); PPO timing covers 23/30 seeds
           (3 PPO Pong and 4 PPO Breakout seeds permanently lost).}
...
PPO & Pong     &  7/10 & 25.8 & 3.7$^*$ \\
PPO & Breakout &  6/10 & 19.4 & 3.2$^{**}$ \\
PPO & Seaquest & 10/10 & 36.8 & 3.7 \\
\midrule
\multicolumn{2}{l}{DQN total} & 30/30 & 72.5 & --- \\
\multicolumn{2}{l}{PPO total (est.)} & 23/30 & $\approx$105.9$^{***}$ & --- \\
\bottomrule
\end{tabular}
\vspace{0.5em}

\noindent{\small $^*$ 7-seed average; 3 seeds have no timing data.
$^{**}$ 6-seed average; 4 seeds have no timing data.
$^{***}$ Extrapolated: Pong total scaled $\times\frac{10}{7}$, Breakout total scaled $\times\frac{10}{6}$.}
\end{table}
