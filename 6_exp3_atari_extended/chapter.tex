...
30 runs total (2 algorithms $\times$ 3 environments $\times$ 10 seeds). Training timing for the original run was lost due to an instrumentation issue. Reported wall-clock figures are taken from retrained reproducibility runs, which cover all 10 DQN seeds on all three environments. PPO timing is currently incomplete (see Limitations); this affects only the timing analysis, not the evaluation scores, which are complete for all 30 runs.

Retrained DQN wall-clock totals per 10 seeds on RTX~4090: approximately 27.7 hours (Pong), 17.4 hours (Breakout), and 27.4 hours (Seaquest), for an aggregate of $\sim$72.5 hours across 30 DQN runs.
...
\begin{figure}[htbp!]
  ...
  \caption{Learning curves on Pong, 10 seeds per algorithm, 20M steps. PPO converges near the
...
\begin{figure}[htbp!]
  ...
  \caption{Learning curves on Breakout, 10 seeds per algorithm, 20M steps. DQN achieves a
...
\begin{figure}[htbp!]
  ...
  \caption{Learning curves on Seaquest, 10 seeds per algorithm, 20M steps. Both algorithms
...
\begin{figure}[htbp!]
  ...
  \caption{Combined normalised learning curves across Pong, Breakout, and Seaquest, 10 seeds per algorithm.}
...
\begin{figure}[htbp!]
  ...
  \caption{Distribution of normalised final scores per environment. DQN is concentrated
...
\begin{figure}[htbp!]
  ...
  \caption{Heatmap of normalised final scores by seed (rows) and environment (columns).}
...
\begin{figure}[htbp!]
  ...
  \caption{Box-and-swarm plot of per-seed normalised scores across environments.}
...
\begin{figure}[htbp!]
  ...
  \caption{Cross-environment IQM, mean, and median with 95\% bootstrap CIs.
...
\begin{figure}[htbp!]
  ...
  \caption{Pairwise probability of improvement: $P(\text{DQN}>\text{PPO}) = 0.393$.
...
\begin{figure}[htbp!]
  ...
  \caption{Original vs retrained learning curves. The two runs of each seed are
...
\begin{figure}[htbp!]
  ...
  \caption{Per-seed wall-clock training time for the retrained DQN runs
           (all 10 seeds on all three environments).}
  \label{fig:exp3-timing}
\end{figure}

{\color{red} TODO: Update PPO timing once the PPO Pong retraining (currently running) completes. Current data: DQN all 10 seeds completed --- Pong $\approx$27.7h total, Breakout $\approx$17.4h, Seaquest $\approx$27.4h. PPO timing incomplete (original timing lost; retrained recovery ongoing).}
