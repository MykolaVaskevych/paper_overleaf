...
\chapter[EXP3: Extended Atari Benchmark]{Extended Atari Benchmark}
\label{chap:exp3}
...
\begin{enumerate}
  \item {\color{red} \textbf{H3.1 ---}} \textbf{PPO continues to dominate Pong.} Pong is a policy-gradient-friendly task with a dense reward signal. Extending the budget should not change the ranking.
  \item {\color{red} \textbf{H3.2 ---}} \textbf{DQN continues to edge Breakout.} Breakout rewards value-based exploitation of learned state-action values, and Experiment~2 already showed DQN ahead. More training should let DQN widen the gap.
  \item {\color{red} \textbf{H3.3 ---}} \textbf{Both algorithms make meaningful progress on Seaquest.} Seaquest was chosen as an environment where both algorithms are known to learn non-trivial policies at Nature-paper budgets.
  \item {\color{red} \textbf{H3.4 ---}} \textbf{The PPO-dominance aggregate from Experiment~2 should survive.} Experiment~2 found $P(\text{PPO}>\text{DQN})=0.648$; adding Seaquest and extending the budget should not flip the aggregate.
  \item {\color{red} \textbf{H3.5 ---}} \textbf{Cross-environment aggregates will still be fragile at $M=3$.} A single environment can still dominate the profile if its per-environment IQM is much larger than the others.
\end{enumerate}
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Learning curves, Pong]{Learning curves on Pong, 10 seeds per algorithm, 20M steps. PPO converges near the
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Learning curves, Breakout]{Learning curves on Breakout, 10 seeds per algorithm, 20M steps. DQN achieves a
...
{\color{red} Seaquest is the most demanding environment in this study: the agent must manage an oxygen bar, rescue divers, and fight enemies simultaneously, requiring multi-objective decision-making under time pressure.}
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Learning curves, Seaquest]{Learning curves on Seaquest, 10 seeds per algorithm, 20M steps. Both algorithms
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Combined learning curves]{Combined normalised learning curves across Pong, Breakout, and Seaquest, 10 seeds per algorithm.}
...
Figures~\ref{fig:exp3-heatmap} and~\ref{fig:exp3-boxswarm} provide two complementary views. PPO dominates Pong (normalised scores $0.82$--$1.00$) while Breakout and Seaquest are both near the floor ($0.04$--$0.31$ and $0.02$--$0.03$ respectively). DQN Breakout occupies the mid-range ($0.38$--$0.84$); DQN Pong is wide-spread; both algorithms are near zero on Seaquest in normalised terms.

\begin{figure}[htbp!]
  \centering
  \includegraphics[width=\linewidth]{6_exp3_atari_extended/assets/per_seed_heatmap.png}
  \caption[EXP3: Per-seed score heatmap]{Heatmap of normalised final scores by seed (rows) and environment (columns).}
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Per-seed score spread]{Box-and-swarm plot of per-seed normalised scores across environments.}
...
\begin{table}[htbp!]
\centering
\caption[EXP3: Cross-environment aggregate metrics]{Cross-environment aggregate metrics for Experiment~3 (normalised scores).}
...
\midrule
DQN & $0.443\;[0.369,\;0.521]$ & $0.450\;[0.393,\;0.500]$ & $0.571\;[0.504,\;0.647]$ & $0.550\;[0.500,\;0.607]$ \\
PPO & $0.277\;[0.247,\;0.309]$ & $0.386\;[0.368,\;0.403]$ & $0.161\;[0.121,\;0.204]$ & $0.614\;[0.597,\;0.632]$ \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Cross-environment IQM]{Cross-environment IQM, mean, and median with 95\% bootstrap CIs.
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Performance profiles]{Performance profiles with 95\% bootstrap CIs.}
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Optimality gap]{Optimality gap. Lower is better. DQN 0.550 (CI [0.500, 0.607]),
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Probability of improvement]{Pairwise probability of improvement: $P(\text{DQN}>\text{PPO}) = 0.393$.
...
\begin{figure}[htbp!]
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{Seaquest}
  \end{subfigure}
  \caption[EXP3: Sample efficiency per environment]{Sample efficiency (normalised AUC) per environment at the 20M-step budget.}
  \label{fig:exp3-sample-eff}
\end{figure}

Table~\ref{tab:exp3-sample-eff} reports the normalised AUC values. PPO is dramatically more sample-efficient on Pong ($0.955$ vs $0.359$), reflecting its rapid early convergence. On Breakout, DQN is more efficient ($0.217$ vs $0.117$) --- its value-based learning compounds steadily throughout the budget. On Seaquest, both algorithms are inefficient in normalised terms owing to the environment's extremely high theoretical maximum; DQN ($0.008$) and PPO ($0.024$) both remain far below 1.0 throughout.

\begin{table}[htbp!]
\centering
\caption[EXP3: Sample efficiency AUC]{Sample efficiency (normalised AUC) per environment at the 20M-step budget.}
\label{tab:exp3-sample-eff}
\begin{tabular}{lccc}
\toprule
\textbf{Algorithm} & \textbf{Pong} & \textbf{Breakout} & \textbf{Seaquest} \\
\midrule
PPO & 0.955 & 0.117 & 0.024 \\
DQN & 0.359 & 0.217 & 0.008 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Reproducibility learning curves]{Original vs retrained learning curves. The two runs of each seed are
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Reproducibility final scores]{Original vs retrained final scores. Perfect diagonal agreement
...
\begin{figure}[htbp!]
  ...
  \caption[EXP3: Per-seed wall-clock training time]{Per-seed wall-clock training time (hours) for all 60 seeds.
...
\begin{table}[htbp!]
\centering
\caption[EXP3: Wall-clock training time]{Wall-clock training time per algorithm and environment (20M steps/seed).}
...
PPO's Pong score of 19.78 closely matches the approximately 20 reported by \citet{schulman2017ppo}, suggesting the PPO implementation is well-calibrated on this environment. DQN's Breakout score of 229 moves substantially toward the 401.2 from \citet{mnih2015human} --- which used 50 million frames, a 1,000,000-frame replay buffer, and hyperparameters informally tuned on Breakout (see Section~\ref{subsec:experimental-design}) --- confirming that value-based learning continues to compound with additional experience. On Seaquest, PPO's advantage over DQN (1174 vs 714) is consistent with \citet{defuente2024comparative}, who note that policy gradient methods tend to outperform value-based methods on environments requiring deeper exploration and complex strategy. Both scores remain below the levels in the original papers --- \citet{mnih2015human} reports DQN at 5286 and \citet{schulman2017ppo} shows PPO at approximately 1500--1600 --- consistent with Seaquest requiring substantially more than 20 million steps to approach published performance levels. Direct comparison across papers should be treated with caution due to divergent evaluation protocols \citep{machado2018revisiting}.}

%% ------------------------------------------------------------------
{\color{red} \section{Threats to Validity}}
\label{sec:exp3-threats}

{\color{red} Several limitations constrain the generalisability of these results:}
\begin{itemize}
  ...
  \item \textbf{Training timing recovered via retraining.} Original wall-clock logs were lost due to an instrumentation issue. Full timing for all 60 seeds was recovered by deterministically retraining, which is possible because the pipeline is bitwise reproducible on the target hardware (Section~\ref{sec:exp3-reproducibility}). Evaluation scores are complete for all 30 runs.
\end{itemize}
