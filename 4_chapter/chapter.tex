...
meaningful evaluation scores from that data, and what metrics I computed to compare algorithms.
...
From this trajectory data, I do \emph{not} make algorithm comparisons directly on training rewards
because training behavior (e.g., exploration noise, replay buffer dynamics, on- vs off-policy sampling)
is algorithm-dependent and not a stable measure of generalization. Instead, I define a fixed
...
internally, I report undiscounted $G$ unless otherwise specified by the benchmark.

I use the following index conventions throughout:
...
At each checkpoint $t$, I evaluate a frozen policy for $E$ episodes and store the mean
episodic return:
\[
...
these logged values. At a fixed checkpoint $t$, I can organize these values into a score matrix
\[
...
I fix a single evaluation policy selection rule for all algorithms
...
statistical uncertainty in aggregate summaries, I report \textbf{95\% stratified bootstrap
confidence intervals} for all non-point-estimate metrics. In stratified bootstrap, for each
environment $m$ I resample the set of seeds with replacement to form a bootstrap replicate
of the score matrix; I recompute the statistic on each replicate and take the 2.5\% and
...
For each experiment I report: number of seeds $N$, evaluation episodes $E$,
total training budget and evaluation frequency, evaluation action rule, and hyperparameter
configuration. To support reproducibility and meaningful comparison, I will provide
...
\begin{figure}[htbp!]
  ...
  \caption{Combined normalised learning curves across all three
           ...
           \textcolor{red}{(Regenerate after you will removal of memory)}}
