...
\textcolor{red}{\textbf{TODO:} Retrain DQN$\times$Pong seeds 1--7 with \texttt{n\_envs=16} for consistency.}
...
DQN$\times$Pong appears slower than PPO$\times$Pong
($68{,}542$\,s vs $59{,}569$\,s), but this comparison is confounded:
seeds~1--7 used \texttt{n\_envs=8} rather than~16, roughly halving
their throughput.

\textcolor{red}{\textbf{TODO:} Retrain DQN$\times$Pong seeds 1--7
with \texttt{n\_envs=16}; this will also produce fair timing data.}

\textcolor{red}{\textbf{TODO:} Add timing instrumentation to
\texttt{train.py}/\texttt{run.sh} so all future runs capture
wall-clock time automatically.}
...
I acknowledge the \texttt{n\_envs} mismatch for DQN$\times$Pong
seeds~1--7 (8 vs 16 parallel sub-environments). The evaluation
pipeline truncates to the minimum checkpoint count, ensuring metric
comparability. I expect the effect on final performance to be minimal,
as the total training budget (5M steps) is identical across all seeds.
