...
networks on GPU. The purpose is to test whether the DQN-vs-PPO
...
order \citep{pytorch_reproducibility}. 

To compensate, Experiment~2
uses \emph{statistical validity} via 10 independent seeds rather than
the bitwise reproducibility strategy of Experiment~1. \textbf{\textit{(rephrase this)}}

One procedural note: DQN$\times$Pong seeds~1--7 were trained with
\texttt{n\_envs=8} (8 parallel sub-environments via
\texttt{SubprocVecEnv}), while all remaining runs used
\texttt{n\_envs=16}. The evaluation pipeline truncates to the minimum
checkpoint count across seeds, so all metrics remain comparable. The
impact on learning dynamics is expected to be minimal. 

\textbf{\textit{TODO: redo this seeds if not forget about it, gosh its tiresome}}
...
Raw returns were normalised using the same formula as Chapter~5:
\[
...
\begin{enumerate}
  \item \textbf{PPO should dominate Pong.}
  ...
  \item \textbf{DQN should excel on Breakout.}
  ...
  \item \textbf{Both should substantially exceed random baselines.}
        5M timesteps is a generous budget for two-environment Atari
        training.

  \item \textbf{PPO should be more sample-efficient.}
  ...
  \item \textbf{Higher variance than Experiment~1.}
...
PPO training times were recorded for all 10 Breakout seeds and 3 Pong
seeds (the remaining Pong seeds predated the timing instrumentation).
The total recorded PPO training time was approximately
\textbf{55{,}927\,s ($\approx$15.5~hours)}: Pong averaged
${\sim}5{,}771$\,s/seed and Breakout averaged ${\sim}3{,}861$\,s/seed.
DQN training times were not recorded, as training predated the timing
instrumentation added to the pipeline. Based on similar compute
profiles, total DQN training is estimated at 8--15~hours.
...
PPO nearly solves Pong, achieving a mean raw return of $20.05$ out of
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/learning_curves_pong.png}
  \caption{Learning curves for Pong. PPO converges near the maximum
...
The surprise of this experiment: DQN outperforms PPO on Breakout.
DQN achieves a mean raw return of $43.62$ versus PPO's $32.51$, with
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/learning_curves_breakout.png}
  \caption{Learning curves for Breakout. DQN slightly outperforms PPO,
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/combined_learning_curves.png}
  \caption{Combined normalised learning curves for Pong and Breakout.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/score_distribution.png}
  \caption{Distribution of normalised scores per environment. PPO Pong
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/per_seed_heatmap.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/per_seed_boxswarm.png}
  \caption{Box-and-swarm plot of per-seed normalised scores. DQN
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/final_performance.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/performance_profile.png}
  \caption{Performance profiles with 95\% bootstrap CIs. PPO's flat
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/optimality_gap.png}
  \caption{Optimality gap with 95\% bootstrap CIs. Both algorithms
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{5_chapter/assets/sample_efficiency_pong.png}
    \caption{Pong}
    \label{fig:atari-se-pong}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{5_chapter/assets/sample_efficiency_breakout.png}
    \caption{Breakout}
    \label{fig:atari-se-breakout}
  \end{subfigure}
  \caption{Sample efficiency (normalised AUC) per environment. PPO is
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\linewidth]{5_chapter/assets/poi_heatmap.png}
  \caption{Probability-of-improvement heatmap. PPO wins overall
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{5_chapter/assets/timing_analysis.png}
  \caption{Wall-clock evaluation time per algorithm and environment.
...
Both algorithms' Breakout performance is poor in absolute terms.
The original DQN paper \citep{mnih2015human} trained for 50M frames
(approximately $12.5$M steps with frame-skip 4)---$2.5\times$ our
budget. The low scores suggest both algorithms are still in the
early stages of learning Breakout's complex dynamics.
...
$0.529$ vs $0.130$), but the picture is more nuanced. The key
difference is the observation space: pixel-based observations with
longer horizons and sparser rewards (Breakout) create conditions
where DQN's replay buffer provides a genuine advantage. DQN's
failure on classic-control was a budget issue---200K steps with
SB3's default $\varepsilon$-greedy schedule was insufficient. On
Atari with 5M steps, DQN has enough data to show its strengths,
at least on Breakout.
...
The \texttt{n\_envs} mismatch for DQN$\times$Pong seeds~1--7 (8 vs
16 parallel sub-environments) is acknowledged. The evaluation pipeline
truncates to the minimum checkpoint count, ensuring metric
comparability. The effect on final performance is expected to be
minimal, as the total training budget (5M steps) is identical across
all seeds.
...
\begin{itemize}
  ...
  \item \textbf{Missing DQN training times.} DQN training predated
        the timing instrumentation, so training cost cannot be compared
        fairly between algorithms.
\end{itemize}
