...
\begin{table}[htbp!]
\centering
\caption[SB3 default hyper-parameters]{Default Stable-Baselines 3 hyper-parameters used for all algorithms. No algorithm-specific tuning was performed to ensure fair comparisons.}
...
\subsection{Why Local Training \textcolor{red}{remove?}}
\label{sec:why-local}
...
\begin{enumerate}
  \item \textbf{RL bottlenecks on CPU, not GPU.} \textcolor{red}{(REMOVE?)}
  ...
  \item \textbf{Deterministic seeding requires CPU-only execution. \textcolor{red}{(REMOVE?)}}
...
\begin{enumerate}
    ...
    \item \textbf{PPO and A2C should produce competitive results.}
    ...
    \item \textbf{RPPO's long short-term memory (LSTM) is unlikely to help and may hurt performance.}
    ...
    \item \textbf{Difficulty ordering: CartPole $<$ Acrobot $<$ LunarLander.}
...
\begin{table}[htbp!]
\centering
\caption[EXP1: Training time per algorithm]{Wall-clock training time (seconds) per algorithm, summed over 15 seeds per environment.}
...
\begin{figure}[htbp!]
  ...
  \caption[Learning curves for CartPole-v1.] {Learning curves for CartPole-v1.PPO and A2C converge to the maximum return of 500 within 100K steps while DQN and QR-DQN fail to learn within the 200K-step budget, with QR-DQN barely moving.}
...
\begin{figure}[htbp!]
  ...
  \caption{Learning curves for Acrobot-v1. A2C and PPO both converge near the optimal return ($\approx -85$). RPPO and DQN both fail entirely, remaining near the episode truncation limit of $-500$ throughout training.}
...
\begin{figure}[htbp!]
  ...
  \caption[Combined normalized learning curves across all three
           environments]{Combined normalized learning curves across all three
