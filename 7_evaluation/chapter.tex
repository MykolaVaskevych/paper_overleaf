...
\begin{itemize}
  ...
  buffer.
  \item \textbf{Timing data recovered via retraining.}
  Original wall-clock logs for Experiment~3 were lost due to an instrumentation issue. Per-seed timing was recovered by deterministically retraining all 60 seeds, which is possible because the pipeline is bitwise reproducible on the target hardware. Full timing is reported in Section~\ref{sec:exp3-timing}.
\end{itemize}
